  ...
  if (!offer.isUpgrade) {
    ...
    return (
      <div className="pointer-events-auto animate-in slide-in-from-bottom-4 fade-in duration-300 flex flex-col items-center gap-2 pb-3">
        <span className="text-xs font-bold uppercase tracking-wider text-white/70">Choose a Perk</span>
...
function PerkCard({ perk, onClick }: { perk: (typeof PERKS)[PerkId]; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group relative flex w-[130px] cursor-pointer flex-col items-center gap-1.5 rounded-xl border bg-black/70 px-3 py-3 backdrop-blur-sm transition-all hover:scale-105 hover:bg-black/80"
      style={{
    ...
    >
      ...
      >
        <Icon size={22} style={{ color: colors.border }} />
      </div>
      <span className="text-[11px] font-bold text-white">{perk.name}</span>
      <span className="text-center text-[9px] leading-tight text-white/60">{perk.description}</span>
      <span
        className="mt-auto text-[8px] font-bold uppercase tracking-wider"
...
function UpgradeCard({
  ...
  return (
    ...
    >
      ...
      >
        <ToIcon size={22} style={{ color: toColors.border }} />
      </div>
      <span className="text-[9px] text-white/50 line-through">{from.name}</span>
      <span className="text-[11px] font-bold text-white">{to.name}</span>
      <span className="text-center text-[9px] leading-tight text-white/60">{to.description}</span>
      <span
        className="mt-auto text-[8px] font-bold uppercase tracking-wider"
...
function JollyCard({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group relative flex w-[130px] cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-purple-500/50 bg-black/70 px-3 py-3 backdrop-blur-sm transition-all hover:scale-105 hover:border-purple-400/70 hover:bg-black/80"
      style={{
    ...
    >
      ...
      <span className="text-[11px] font-bold text-purple-300">Mystery</span>
      <span className="text-center text-[9px] leading-tight text-white/50">Random perk — feeling lucky?</span>
    </button>
  );
}
