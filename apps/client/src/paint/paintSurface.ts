...
// --- per-class, per-part registry ---
// Head and body are separate meshes, so each gets its own surface: independent
// skin color + paint. The keys match the paintable placeholder part names.
...
export function getPaintSurface(characterClass: CharacterClass, part: PaintPart): PaintSurface {
...
/** The body + head textures for a class, for handing to a model's `paint` prop. */
export function paintTexturesFor(characterClass: CharacterClass): PaintTextures {
  return {
    body: getPaintSurface(characterClass, 'body').texture,
    head: getPaintSurface(characterClass, 'head').texture,
  };
}
